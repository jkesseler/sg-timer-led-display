...
export function startServerMqttSubscriber(): void {
  ...
  client.on('connect', () => {
    ...
    client?.subscribe('timer/+/session/started', { qos: 1 }, (error) => {
      if (error) console.error('[mqtt-server] failed to subscribe to session/started', error)
    })
    client?.subscribe('timer/+/session/stopped', { qos: 1 }, (error) => {
      if (error) console.error('[mqtt-server] failed to subscribe to session/stopped', error)
    })
  })
