...
TEST_F(SpecialPieProtocolTest, ShotDetected_TimeConversion) {
  ...
  // Shot 1 (first of the session) at 3.45 seconds (3 sec, 45 centiseconds)
  uint8_t pkt[] = { 0xF8, 0xF9, 0x36, 0x00, 0x03, 0x2D, 0x01, 0x00, 0xF9, 0xF8 };
  device.processTimerData(pkt, sizeof(pkt));

  EXPECT_TRUE(callbackCalled);
  EXPECT_EQ(receivedShot.shotNumber, 1);              // wire is already 1-based
...
TEST_F(SpecialPieProtocolTest, ShotDetected_SplitCalculation) {
  ...
  uint8_t shot1[] = { 0xF8, 0xF9, 0x36, 0x00,  0x02, 0x1E, 0x01, 0x00,  0xF9, 0xF8 };
  device.processTimerData(shot1, sizeof(shot1));
  ASSERT_TRUE(callbackCalled);
  ...
  uint8_t shot2[] = { 0xF8, 0xF9, 0x36, 0x00,  0x03, 0x05, 0x02, 0x00,  0xF9, 0xF8 };
  device.processTimerData(shot2, sizeof(shot2));
...
TEST_F(SpecialPieProtocolTest, ShotDetected_CentisecondsBorrowFromSeconds) {
  ...
  uint8_t shot1[] = { 0xF8, 0xF9, 0x36, 0x00,  0x02, 0x50, 0x01, 0x00,  0xF9, 0xF8 };
  device.processTimerData(shot1, sizeof(shot1));
  ASSERT_TRUE(callbackCalled);
...
TEST_F(ASNTrackerProtocolTest, ShotDetected_TimeConversion) {
  ...
  //                                           5s    50cs   shot#2 (wire, 1-based)
  device.processTimerData(pkt, sizeof(pkt));

  EXPECT_TRUE(callbackCalled);
  EXPECT_EQ(receivedShot.shotNumber, 2);            // wire is already 1-based
...
TEST_F(SpecialPieMacProtocolTest, ShotDetected_FirstShot) {
  uint8_t pkt[] = { 0xF8, 0xF9, 0x36, 0x00,  0x04, 0x19, 0x01, 0x00,  0xF9, 0xF8 };
  //                                           4s    25cs   shot#1
  device.processTimerData(pkt, sizeof(pkt));
...
TEST_F(SpecialPieMacProtocolTest, ShotDetected_SplitCalculation) {
  // Shot 1 at 2.50s
  uint8_t s1[] = { 0xF8, 0xF9, 0x36, 0x00,  0x02, 0x32, 0x01, 0x00,  0xF9, 0xF8 };
  device.processTimerData(s1, sizeof(s1));
  EXPECT_EQ(receivedShot.absoluteTimeMs, 2500u);

  // Shot 2 at 3.20s → split = 700 ms
  uint8_t s2[] = { 0xF8, 0xF9, 0x36, 0x00,  0x03, 0x14, 0x02, 0x00,  0xF9, 0xF8 };
  device.processTimerData(s2, sizeof(s2));
...
TEST_F(SpecialPieMacProtocolTest, ShotResetsOnNewSession) {
  ...
  uint8_t s2[] = { 0xF8, 0xF9, 0x36, 0x00,  0x01, 0x00, 0x01, 0x00,  0xF9, 0xF8 };
  device.processTimerData(s2, sizeof(s2));
  EXPECT_TRUE(callbackCalled);
  EXPECT_TRUE(receivedShot.isFirstShot);
...
TEST_F(SpecialPieMacProtocolTest, RealCaptureReplay_DecodesShotsAndSplits) {
  ...
  const Frame frames[] = {
    {{0xF8,0xF9,0x36,0x00, 0x0C,0x4A,0x02,0x0B, 0xF9,0xF8},  2, 12740,    0, true},
    {{0xF8,0xF9,0x36,0x00, 0x0C,0x4B,0x03,0x0B, 0xF9,0xF8},  3, 12750,   10, false},
    {{0xF8,0xF9,0x36,0x00, 0x0E,0x22,0x04,0x0B, 0xF9,0xF8},  4, 14340, 1590, false},
    {{0xF8,0xF9,0x36,0x00, 0x0E,0x26,0x05,0x0B, 0xF9,0xF8},  5, 14380,   40, false},
    {{0xF8,0xF9,0x36,0x00, 0x0F,0x10,0x06,0x0B, 0xF9,0xF8},  6, 15160,  780, false},
    {{0xF8,0xF9,0x36,0x00, 0x10,0x24,0x07,0x0B, 0xF9,0xF8},  7, 16360, 1200, false},
    {{0xF8,0xF9,0x36,0x00, 0x10,0x28,0x08,0x0B, 0xF9,0xF8},  8, 16400,   40, false},
    {{0xF8,0xF9,0x36,0x00, 0x10,0x33,0x09,0x0B, 0xF9,0xF8},  9, 16510,  110, false},
    {{0xF8,0xF9,0x36,0x00, 0x11,0x53,0x0A,0x0B, 0xF9,0xF8}, 10, 17830, 1320, false},
    {{0xF8,0xF9,0x36,0x00, 0x11,0x55,0x0B,0x0B, 0xF9,0xF8}, 11, 17850,   20, false},
    {{0xF8,0xF9,0x36,0x00, 0x14,0x0B,0x0C,0x0B, 0xF9,0xF8}, 12, 20110, 2260, false},
    {{0xF8,0xF9,0x36,0x00, 0x14,0x0C,0x0D,0x0B, 0xF9,0xF8}, 13, 20120,   10, false},
  };
...
TEST(CrossDevice, ShotIndexBaseMatchesDeviceWireFormat) {
  // Guards the per-device numbering constants against silent drift. The frame
  // protocol devices report the first shot of a session as 1; the SG Timer
  // reports it as 0. Everything downstream sees 1-based numbers either way.
  EXPECT_EQ(SpecialPieM1A2F::SHOT_INDEX_BASE, 1);
  EXPECT_EQ(SpecialPieM1A2Plus::SHOT_INDEX_BASE, 1);
  EXPECT_EQ(ASNTracker::SHOT_INDEX_BASE, 1);
  EXPECT_EQ(SGTimer::SHOT_INDEX_BASE, 0);
}
